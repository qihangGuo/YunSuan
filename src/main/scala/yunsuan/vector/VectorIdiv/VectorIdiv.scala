...
class VectorIdiv extends Module {
  val Vectorwidth = VIFuParam.VLEN
  private val GroupWidth = 128
  require(Vectorwidth % GroupWidth == 0, s"VectorIdiv: VLEN($Vectorwidth) must be a multiple of $GroupWidth")
  private val Groups = Vectorwidth / GroupWidth
  private val DZeroWidth = Vectorwidth / 8
  val io = IO(new Bundle() {
    ...
    // Divide-by-zero flags, packed in low bits (1 bit per element), max width assumes SEW=8.
    val d_zero = Output(UInt(DZeroWidth.W))
  ...
  /*
    Per 128-bit group:
      I8  : 8*I8DivNr4 + 4*16b + 2*32b + 2*64b => 16 x 8b results
      I16 : 4*16b + 2*32b + 2*64b             => 8  x 16b results
      I32 : 2*32b + 2*64b                      => 4  x 32b results
      I64 : 2*64b                               => 2  x 64b results
   */
  val group_q = Wire(Vec(Groups, UInt(GroupWidth.W)))
  val group_rem = Wire(Vec(Groups, UInt(GroupWidth.W)))
  val group_d_zero = Wire(Vec(Groups, UInt(16.W)))
  val group_finish = Wire(Vec(Groups, Bool()))

  for (g <- 0 until Groups) {
    def grpSlice(x: UInt, hi: Int, lo: Int): UInt = x(g * GroupWidth + hi, g * GroupWidth + lo)
    ...
    for (i <- 0 until 8) {
      val begin = i * 8
      val end = (i + 1) * 8 - 1
      val divide_8 = Module(new I8DivNr4().suggestName(s"g${g}_8bit_divide_${i}"))
      ...
      divide_8.io.dividend := grpSlice(x_reg, end, begin)
      divide_8.io.divisor := grpSlice(d_reg, end, begin)
    ...
    // I16
    val divide_16_q_result = Wire(Vec(4, UInt(16.W)))
    ...
    for (i <- 0 until 4) {
      ...
      val divide_16_dividend =
        Mux(sew_hb(0), grpSlice(x_reg, end_I8, begin_I8), grpSlice(x_reg, end_I16, begin_I16))
      val divide_16_divisor =
        Mux(sew_hb(0), grpSlice(d_reg, end_I8, begin_I8), grpSlice(d_reg, end_I16, begin_I16))
      val divide_16 = Module(new SRT16Divint(16).suggestName(s"g${g}_16bit_divide_${i}"))
    ...
    for (i <- 0 until 2) {
      ...
      val divide_32_dividend =
        Mux(
          sew_hb(0),
          grpSlice(x_reg, end_I8, begin_I8),
          Mux(sew_hb(1), grpSlice(x_reg, end_I16, begin_I16), grpSlice(x_reg, end_I32, begin_I32))
        )
      val divide_32_divisor =
        Mux(
          sew_hb(0),
          grpSlice(d_reg, end_I8, begin_I8),
          Mux(sew_hb(1), grpSlice(d_reg, end_I16, begin_I16), grpSlice(d_reg, end_I32, begin_I32))
        )
      val divide_32 = Module(new SRT16Divint(32).suggestName(s"g${g}_32bit_divide_${i}"))
    ...
    for (i <- 0 until 2) {
      ...
      val divide_64_dividend =
        Mux(
          ...
          Mux(
            sew_hb(1),
            grpSlice(x_reg, end_I16, begin_I16),
            Mux(sew_hb(2), grpSlice(x_reg, end_I32, begin_I32), grpSlice(x_reg, end_I64, begin_I64))
          )
        )
      val divide_64_divisor =
        Mux(
          ...
          Mux(
            sew_hb(1),
            grpSlice(d_reg, end_I16, begin_I16),
            Mux(sew_hb(2), grpSlice(d_reg, end_I32, begin_I32), grpSlice(d_reg, end_I64, begin_I64))
          )
        )
      val divide_64 = Module(new SRT16Divint(64).suggestName(s"g${g}_64bit_divide_${i}"))
    ...
    group_finish(g) := divide_8_finish.reduce(_ & _) &
      divide_16_finish.reduce(_ & _) &
      divide_32_finish.reduce(_ & _) &
    ...
    group_d_zero(g) := Mux(
      ...
      Mux(
        ...
        Mux(sew_hb(2), Cat(0.U(12.W), divide_64_d_zero.asUInt, divide_32_d_zero.asUInt), Cat(0.U(14.W), divide_64_d_zero.asUInt))
      )
    )
    ...
    group_rem(g) := Mux(
      sew_hb(0),
      Cat(divide_64_I8_rem.asUInt, divide_32_I8_rem.asUInt, divide_16_I8_rem.asUInt, divide_8_rem_result.asUInt),
      Mux(
        sew_hb(1),
  ...
  val div_out_d_zero_result = Wire(UInt(DZeroWidth.W))
  val div_out_d_zero_result_reg = RegEnable(div_out_d_zero_result, stateReg(divide))

  finish := group_finish.reduce(_ & _)

  div_out_q_result := Cat(group_q.reverse)
  div_out_rem_result := Cat(group_rem.reverse)

  val dzero_sew0 = Cat(group_d_zero.reverse) // 16b per group
  val dzero_sew1 = Cat(group_d_zero.reverse.map(_(7, 0))) // 8b per group
  val dzero_sew2 = Cat(group_d_zero.reverse.map(_(3, 0))) // 4b per group
  val dzero_sew3 = Cat(group_d_zero.reverse.map(_(1, 0))) // 2b per group
  div_out_d_zero_result := Mux(
    sew_hb(0),
    dzero_sew0,
    Mux(sew_hb(1), ZeroExt(dzero_sew1, DZeroWidth), Mux(sew_hb(2), ZeroExt(dzero_sew2, DZeroWidth), ZeroExt(dzero_sew3, DZeroWidth)))
  )
